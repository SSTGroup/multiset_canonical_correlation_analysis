...
def plot_true_estimated_results_for_paper(folder1, folder2, n_montecarlo, save=False):
    ...
    axes[0].set_xlabel(r'(a) True covariance matrices', fontsize=12)
    ...
    axes[1].set_xlabel(r'(b) Estimated covariance matrices', fontsize=12)
    ...
    if save:
        plt.tight_layout()
        plt.subplots_adjust(wspace=0.2)
        matplot2tikz.save(f'joint_isi.tex', encoding='utf8', axis_width='7.5cm', axis_height='5cm',
                          standalone=True)
        plt.savefig(f'joint_isi.pdf')
    else:
